                                 ...
                                 inout float4 vertex, inout float3 normal, inout float2 frameUVs, inout float4 viewPos)
{
    ...
    frameUVs.xy = ((tileIndex + (uvExpansion * fractionsUVscale + 0.5)) * (_AlbedoAlpha_TileParam.y +
        _AlbedoAlpha_TileParam.x * 2)) / _AlbedoAlpha_TileParam.zw;
...
inline void SphereImpostorFragment(float2 frameUVs, float4 fitSize, float4 viewPos,
                                   inout SurfaceOutputStandardMetallic o, out float4 clipPos, out float3 worldPos)
{
    // albedo alpha
